...
public:
    ...
    lastCoul = firstCoul->prevcoul;
    if (firstCoul != nullptr && lastCoul != nullptr && firstCoul != lastCoul) {
        current = firstCoul;
        do {
            Lstcoul tempCoul = current->coul;
            std::string tempDisplay = current->display;
            current->coul = current->prevcoul->coul;
            current->display = current->prevcoul->display;
            current->prevcoul->coul = tempCoul;
            current->prevcoul->display = tempDisplay;
    ...
    void restorecouls() {
        ...
        void shiftform(Lstform form){
            ...
            lastForm = firstForm->prevform;

            if (firstForm != nullptr && lastForm != nullptr && firstForm != lastForm) {
               current = firstForm;
                while (current != lastForm) {
                    Lstform tmpForm = current->form;
                    std::string tmpDisplay = current->display;
                    current->form = current->nextform->form;
                    current->display = current->nextform->display;
                    current->nextform->form = tmpForm;
                    current->nextform->display = tmpDisplay;
    ...
    if (board->nbPieces >= 3) {
        ...
        while (currentPiece->next != board->head) {
            int listcoul = 1; 
            int listform = 1;
            Piece* current = currentPiece->next;
            ...
            int ListLarger = (listcoul > listform) ? listcoul : listform;
